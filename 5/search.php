...
<?php
	...
	if(!empty($was) && empty($wo))
	{
		...
		if($type=="branche")
		{
			$findData=mysql_fetch_array(mysql_query("SELECT * FROM find WHERE findName='$was'"));
			$company_count=mysql_fetch_array(mysql_query("SELECT COUNT(companyId) as total FROM company WHERE findId='".$findData['findId']."'"));
			$company_count=$company_count['total'];
			$pageNumber=ceil($company_count/10);				
			$limit=$page*10;
			$company=mysql_query("SELECT * FROM company WHERE findId='".$findData['findId']."' ORDER BY companyName ASC LIMIT $limit,10");
		}
	...
	if(empty($was) && !empty($wo))
	{
		$otherData=mysql_fetch_array(mysql_query("SELECT * FROM other WHERE otherName='$wo' OR postalCode='$wo'"));
		$company_count=mysql_fetch_array(mysql_query("SELECT COUNT(companyId) as total FROM company WHERE otherId='".$otherData['otherId']."'"));
		$company_count=$company_count['total'];
		$pageNumber=ceil($company_count/10);				
		$limit=$page*10;
        	$company=mysql_query("SELECT * FROM company WHERE otherId='".$otherData['otherId']."' ORDER BY companyName ASC LIMIT $limit,10");	
	}
	...
	{	
		if($type=="branche")
		{
			$findData=mysql_fetch_array(mysql_query("SELECT * FROM find WHERE findName='$was'"));
			$otherData=mysql_fetch_array(mysql_query("SELECT * FROM other WHERE otherName='$wo' OR postalCode='$wo'"));
 		        $company_count=mysql_fetch_array(mysql_query("SELECT COUNT(companyId) AS total FROM company WHERE findId='".$findData['findId']."'AND otherId='".$otherData['otherId']."'"));
			$company_count=$company_count['total'];
			$pageNumber=ceil($company_count/10);				
			$limit=$page*10;
			$company=mysql_query("SELECT * FROM company WHERE findId='".$findData['findId']."' AND otherId='".$otherData['otherId']."' 
												ORDER BY companyName ASC LIMIT $limit,10");
		}		
		else if($type=="name")
		{
			$otherData=mysql_fetch_array(mysql_query("SELECT * FROM other WHERE otherName='$wo' OR postalCode='$wo'"));
			$company_count=mysql_fetch_array(mysql_query("SELECT COUNT(companyId) AS total FROM company WHERE companyName LIKE '$was%'AND otherId='".$otherData['otherId']."'"));
			$company_count=$company_count['total'];
			$pageNumber=ceil($company_count/10);				
			$limit=$page*10;
			$company=mysql_query("SELECT * FROM company WHERE companyName LIKE ('$was%') AND otherId='".$otherData['otherId']."' 
												ORDER BY companyName ASC LIMIT $limit,10");	
		}
	}
...
<div id="content">
	...
	<div id="container">
        ...
        <div id="map">
            <?php
            	while($fetch=mysql_fetch_array($company)): $otherId=$fetch['otherId'];
				$companyId=$fetch['companyId'];
				$otherDetail=mysql_fetch_array(mysql_query("SELECT * FROM other WHERE otherId='$otherId'"));
